...
- `npm run build` (= `node build.js`) — regenerates `index.html`, `blog/<slug>/index.html` and `meet/index.html` from `content/`. Success prints `built index.html + N posts + meet/`. Run after any change to `content/` or the templates in `build.js`.
...
- `/meet/` — a group scheduler (LettuceMeet-style: pick days and hours, share the link, everyone paints their free slots, heatmap + best times). `build.js` emits only the shell (`meet/index.html`); `meet.js` renders everything at runtime. Storage is chosen at boot:
  - **Store mode** — when `content/site.json` → `meet.supabaseUrl` / `meet.supabaseAnonKey` are set (the build injects them as `data-store-*` on `.meet`). Events and answers live in the two tables from `supabase/meet.sql` (run it once in the Supabase SQL editor; it also sets the row-level-security policies: anyone can read, create events, add or change an answer; nobody can delete). The page talks PostgREST with plain `fetch`, no SDK. Links are `/meet/#<12-char id>`, writes are debounced upserts keyed by `(event_id, lower(name))`, and the page re-syncs every 15s while visible plus on focus. The anon key is public by design; safety is the policies. Free Supabase projects pause after a week idle and must be unpaused in the dashboard — the page shows a retry message when that happens.
  - **Link mode** — no store configured: the event definition **and every response** are base64url JSON in the URL hash, so each saved answer changes the link and the newest link must be passed on. `localStorage` remembers responses per event id and merges them back in, so the organizer's browser acts as the aggregator when links diverge.
  - Either way, times are the organizer's wall clock with the timezone shown as a label (no conversion), and a name is the identity.
- Generated output — **never edit by hand**: `index.html`, `blog/` and `meet/` (the build deletes and recreates `blog/` every run, so removed or renamed posts leave no stale pages).
- Hand-maintained runtime assets: `styles.css` (all styling, including design tokens), `script.js` (newsletter label morph; loaded on every page), `widgets.js` (post demo widgets; post pages only), `meet.js` (the scheduler; `/meet/` only), `fonts/`.
...
- `/meet/` grid painting uses pointer events with `setPointerCapture` + `elementFromPoint` and `touch-action: none` while editing (so touch drags paint instead of scrolling). Keyboard toggles go through `click` with `e.detail === 0` so a pointer click doesn't double-toggle. The URL is updated with `history.replaceState`, so `hashchange` only fires on real navigation (create, back/forward).
...
Project links (`#`), X/GitHub usernames and the empty `meet` store config in `content/site.json`, and the newsletter backend.
