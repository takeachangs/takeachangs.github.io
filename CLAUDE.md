...
- `/meet/` — a link-only group scheduler (LettuceMeet-style: pick days and hours, share the link, everyone paints their free slots, heatmap + best times). `build.js` emits only the shell (`meet/index.html`); `meet.js` renders everything at runtime. There is no backend: the event definition **and every response** are base64url JSON in the URL hash, so each person's saved answer changes the link and the newest link must be passed on. `localStorage` remembers responses per event id and merges them back in, so the organizer's browser acts as the aggregator when links diverge. Times are the organizer's wall clock with the timezone shown as a label (no conversion). Backend hook point: the `ponytail:` comment at the top of `meet.js`.
